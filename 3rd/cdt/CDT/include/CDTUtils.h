...
struct CDT_EXPORT Triangle
{
    ...
    /// @note Returns (noNeighbor, noVertex) if vertex is not in triangle
    std::pair<TriInd, VertInd> next(const VertInd i) const
    {
        // Safe check: return invalid values instead of asserting
        if(vertices[0] != i && vertices[1] != i && vertices[2] != i)
        {
            return std::make_pair(noNeighbor, noVertex);
        }
        if(vertices[0] == i)
        {
