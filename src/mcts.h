...
namespace coacd
{
  ...
  void ComputeAxesAlignedClippingPlanes(Model &m, const int mcts_nodes, std::vector<Plane> &planes, bool shuffle = false);
...
#include <memory>
  ...
  double default_policy(Node *node, Params &params, std::vector<Plane> &current_path);
  ...
  void ComputeAxesAlignedClippingPlanes(Model &m, const int mcts_nodes, std::vector<Plane> &planes, bool shuffle);
